...
const FROM    = process.env.EMAIL_FROM || 'ClearBudget <onboarding@resend.dev>';
...
export async function sendVerificationEmail(to: string, firstName: string, token: string) {
  ...
  const { data, error } = await resend.emails.send({
    from: FROM,
    to,
    subject: 'Verify your ClearBudget email',
    html: `
      <div style="font-family:sans-serif;max-width:480px;margin:0 auto;padding:32px 24px">
        ...
        <p style="color:#888;font-size:12px;margin-top:24px">
          This link expires in 24 hours. If you didn't create a ClearBudget account, you can safely ignore this email.
        </p>
      </div>
    `,
  });
